...
const account1 = {
  ...
  movementsDates: [
    ...
    '2024-03-22T17:01:17.194Z',
    '2024-03-23T23:36:17.929Z',
...
const startLogOutTimer = function(){
  const tick = function(){
    ...
    //when timer reaches zero, stop timer and log out user
    if(time === 0){
      clearInterval();
  ...
  //setting the time 10 minutes
  let time = 10*60;
  //call the function imediately the timer every second
  tick();
...
const calcDisplayBalance = function(acc, locale=navigator.languages){
...
const transferFunds = function(amt, tAcc, rAcc){
  if(amt > 0 && tAcc.balance >= amt && rAcc && rAcc?.username !== tAcc.username){
    ...
    rAcc?.movementsDates.push(new Date().toISOString);
  }
};
...
const requestLoan = function(lnAmt, acc){
  ...
  if(lnAmt > 0 && acc.movements.some(mov => mov >= 0.1*lnAmt)){
    setTimeout(function(){ // simulate a time period passing to receive funds
